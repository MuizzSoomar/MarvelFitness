...
class ListRewardsComponent extends Component {
  ...
  redeemReward = () => {
    this.closeModal();
    ListService.sendEmail(
      this.state.selectedReward.reward_id,
      this.state.customer.user_id
    ).then(response => {});

    let new_balance =
      Number(this.state.customer.rewards_balance) -
      Number(this.state.selectedReward.value);
  ...
  refreshRewards() {
    ListService.getAllRewards().then(response => {
      this.setState(() => {
        return {
